...
Last updated: **PR26 — Critical pre-deploy code audit & invariant verification (no deploy, no live order; static checks + focused tests + two hardening fixes).**
...
**Remaining risks (documented, not blocking).** A few crash-recovery scenarios are
architecturally sound but not exercised by an explicit end-to-end test (e.g. "PlaceOrder
succeeds on the venue but the completion tx rolls back" — covered conservatively by the
sweeper → `DEAD` + `NEEDS_RECONCILE` and the reconciler's positive-identification path).
These need a live venue or a fault-injecting fake to test end-to-end and are out of scope for
a venue-free audit (rule #3). The first real venue order remains an operator action gated by
the full PR20–PR25 stack.
...
**Remaining:** bespoke UIs for credential provisioning + operator
...
- **PR26 — pre-deploy audit found no critical bug; invariants are now statically enforced**:
  ...
  float). No live order sent; no API key needed.
