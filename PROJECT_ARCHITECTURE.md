...
**Pre-send payload validation (`BuyIntentPayload.Validate`, PR10 correction #5).** Before
the executor marks the request IN_FLIGHT or calls `PlaceOrder`, it validates the decoded
intent: `simulated_ioc=true`, `side=buy`, `order_type=limit`, `intended_price > 0`,
`intended_quantity > 0`, `local_client_order_id` non-empty. (Without this,
`decimalOrZero` would silently turn a bad price/qty string into a **0-value order**.) On
failure the executor does NOT send and does NOT mark IN_FLIGHT — it resolves cleanly via
`OnPlaceRejected` (request FAILED, order + cycle FAILED, lock released; nothing was placed,
so there is no exposure).
